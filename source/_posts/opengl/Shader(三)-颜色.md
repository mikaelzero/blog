...
2. 之后 c.x \* 6.0, 把 Hue 从 0-1 映射到 0-6
   - 将 Hue 从 0-1 映射到 0-6 ,是为了便于计算出对应的 RGB 值
   - 色相环是一个将色彩等间隔分布在圆周上的模型。其中主要有 6 种主色:红绿蓝青紫橙。将 Hue 映射到 0-6 ,就代表色相环上这 6 种主色的索引。通过这个索引,便可以计算出对应的 RGB 值。
   - 色相环模型是这样的:红色为 0°（360°）；黄色为 60°；绿色为 120°；青色为 180°；蓝色为 240°；品红色为 300°
3. floor 函数用于取 f 的整数部分 i，表示色相值处于 RGB 颜色空间的哪个区间。具体来说，将色相值乘以 6 之后，得到的值在 [0, 6) 内。floor 函数将这个值向下取整，得到一个整数，表示该值处于 RGB 颜色空间的哪个区间。例如，如果色相值为 0.75，将其乘以 6 得到 4.5。floor 函数将 4.5 向下取整，得到 4，表示该值处于 RGB 颜色空间的第 4 个区间。因此，可以根据这个区间的编号来计算出对应的 RGB 值。

分割线---

4. 加上 vec3(0.0, 4.0, 2.0)代表了这个分量每个都加上前面 c.x \* 6.0 的值 5. 取余数 mod(......, 6) ,将向量的每个分量都限制在 0-6 的数值,这就是 Hue 映射到 0-6 后,对应的色轮主色角度索引。然后,Hue 值代表色相环上的角度。如:
   - 1/6 到 2/6 之间,索引为 1(对应黄色)
   - 2/6 到 3/6 之间,索引为 2(对应绿色)
   - 3/6 到 4/6 之间,索引为 3(对应青色)
   - 4/6 到 5/6 之间,索引为 4(对应蓝色)
   - 5/6 到 1 之间,索引为 5(对应紫色)
5. 根据索引,使用分段函数计算出 RGB 值，S 代表饱和度:
   - 如果索引为 0:R = 1,G = 1-S,B = 1-S(对应红色)
   - 如果索引为 1:R = 1-S,G = 1,B = 1-S(对应黄色)
   - 如果索引为 5:R = 1-S,G = 1-S, B = 1(对应紫色)
