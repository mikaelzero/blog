...
vec4 result = color1 + color2; // 结果是 (1.0, 1.0, 0.0, 2.0)，黄色（注意结果的 alpha 值为 2.0，通常需要将其限定在 0.0 到 1.0 之间）
```

### 相减

将两个颜色向量按分量相减。这种操作较少用于颜色计算，因为它可能产生不符合预期的效果。结果颜色的每个分量都是输入颜色分量的差。这种操作可以使颜色变暗。例如：

```glsl
vec4 color1 = vec4(1.0, 0.0, 0.0, 1.0); // 红色
vec4 color2 = vec4(0.5, 0.5, 0.5, 1.0); // 灰色
vec4 result = color1 - color2; // 结果是 (0.5, -0.5, -0.5, 0.0)，暗红色（注意结果中的负值和 alpha 值为 0.0，通常需要将其限定在 0.0 到 1.0 之间）
```

### 相乘

将两个颜色向量按分量相乘。这种操作通常用于计算光照效果，例如漫反射和镜面反射。结果颜色的每个分量都是输入颜色分量的乘积。这种操作可以加强或减弱颜色的强度。例如：

```glsl
vec4 color1 = vec4(1.0, 0.0, 0.0, 1.0); // 红色
vec4 color2 = vec4(0.5, 1.0, 0.5, 1.0); // 绿色
vec4 result = color1 * color2; // 结果是 (0.5, 0.0, 0.0, 1.0)，暗红色
```

### 叠加

比如有一个基础颜色为浅褐色，是撑满整个区域的，这时候如果想在某一个区域内显示其他颜色，比如红色的话，可以将原来的颜色减去之后，再加上新的颜色，比如这样:

```glsl
color =color-beige + red;
```
