...
在分析这段代码的逻辑之前，需要先了解一些知识点。

首先,色相环模型是这样的:Red(0 度) → Yellow(60 度) → Green(120 度) → Cyan(180 度)→ Blue(240 度) → Magenta(300 度)。

Hue 值通常的范围是 0 到 1，但为了方便计算 RGB 值,我们需要将 Hue 值映射到 0 到 6 之间,对应色相环上 6 个主色的索引。
具体做法是:将 Hue 值乘以 6,这样就映射到 0 到 6 范围内，再取余数 mod 6,限制在 0 到 6 之间。
然后,Hue 值代表色相环上的角度。如:
...
1. 首先传入的 vec3 都是 0 到 1 的范围，这个在传参时已经确定,代表了 HSB 三个值
...
3. 将 Hue 值映射到 0-6 区间,对应色轮上的 6 个主色角度,Hue \* 6, 这样就得到一个 0-6 之间的数值,对应色轮上的 6 个主色,取余数 mod(Hue \* 6, 6) ,也会得到 0-6 的数值,这就是 Hue 映射到 0-6 后,对应的色轮主色角度索引。
4. 根据被映射后的 Hue 值(0-6)和饱和度,计算对应 RGB 值

1) 首先将色调 c.x 从 0-1 映射到 0-6,作为色相环上的索引，通过将 Hue 乘以 6,就得到 0 到 6 之间的数值,对应色相环上的 6 个主色。
2) 根据该索引和饱和度 c.y 计算对应的 RGB 值
   - 限制 rgb 范围在 0 到 1
   - 让 rgb 自乘并缩放,以控制饱和度
   - 使用亮度 c.z 缩放最终的 RGB 值
3) 返回 RGB 值
